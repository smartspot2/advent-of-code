...
subcomps = nx.connected_components(G)
subgraphs = []
for comps in subcomps:
    subg = G.subgraph(comps).copy()
    start = [k for k in '1234' if k in subg.nodes][0]
    keys = frozenset([k for k in subg.nodes if isinstance(k, str) and k.islower()])
    subgraphs.append((subg, start, keys))
...
def dijkstra(graph, source, target):
    ...
    while pq:
        ...
        for v in graph.adj[u].keys():
            if isinstance(v, str) and v.isupper() and (v.lower() not in collected and v.lower() in target):
                continue  # can't go if haven't gotten key yet
            # print(f"{u} -- {v}, weight {graph.edges[(u, v)]['weight']}")
            new_cost = cur_costs[u, collected] + graph.edges[(u, v)]['weight']
...
costs = [min(dijkstra(g, start, keys)) for g, start, keys in subgraphs]
